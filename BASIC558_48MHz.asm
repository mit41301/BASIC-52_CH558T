...
;
;   2023.06.18 I2CSFR のI2C入出力機能を全面書き換え
;
...
;                          After calling, acknowledgment (0/1) received from the receiving side is
;                          entered in status (18H)
...
;                          "loop-number" is the number of dummy loops to wait for
;
;*****************************************************************************
;*                                                                           *
...
;
	CJNE	R3,#HIGH ERAMEND,CR0	; Stop the test at 03FFH/17FFH because
...
;=== CH559T alternative UART0 port setting =====
  ...
  ENDIF
;================
	orl		t2mod,#0C0H		; Set bTMR_CLK, bT2_CLK ( Set Timer2 Clock, MAX Speed = Fsys = 48MHz )
...
setfsys:
	...
	anl	a,#0E0H			;  Clear MASK_SYS_CK_SEL
	orl	a,#6			;  Set MASK_SYS_CK_SEL = 00110B(288MHz/6=48MHz)
...
;*     H-Boehling@gmx.de                                                    *
;****************************************************************************
;* Re-write all I2C routine 2023.06.18
;****************************************************************************
...
;--- ADC command ---
P1_IE		equ	0B9H	; port 1 input enable, 0=enable ADC analog input and disable digital input
ADC_CK_SE	equ	0EFH	; ADC clock divisor setting
ADC_STAT	equ	0F1H	; ADC status
ADC_CTRL	equ	0F2H	; ADC control
ADC_CHANN	equ	0F3H	; ADC channel seletion
ADC_FIFO_L	equ	0F4H	; ReadOnly: FIFO low byte
ADC_FIFO_H	equ	0F5H	; ReadOnly: FIFO high byte
ADC_SETUP	equ	0F6H	; ADC setup
ADC_EX_SW	equ	0F7H	; ADC extend switch control

bADC_IF_ACT		equ	010H	; interrupt flag for a ADC finished, write 1 to clear
bADC_SAMPLE		equ	080H	; automatic or manual sample pulse control, high action
bADC_POWER_EN	equ	004H	; control ADC power: 0=shut down ADC, 1=enable power for ADC
bADC_RESOLUTION	equ	004H	; ADC resolution: 0=10 bits, 1=11 bits
...
; --- SPI command ---
		dw	spistart		; SPISTART command
		dw	spiput			; SPIPUT command
		dw	spiget			; SPIGET command
		dw	SPIROT			; SPIROT command
		dw	spics			; SPICS command
; --- SPI command end ---

; --- ADC command ---
		dw	adcstart		; ADCSTART command
		dw	adcget			; ADCGET command
		dw	adcstop			; ADCSTOP command
; --- ADC command end ---

; --- PULSE command ---
		dw	pulse			; PULSE command
; --- PULSE command end ---
...
;----------------------------------------------------------------------------
;
; === SPI command processer ======================
;
; SPISTART [speed] 
SPISTART:	mov	A,#39H		; get argument "speed"
		lcall	30H			;
		mov	A,#1			;
		lcall	30H			; to R3:R1
		cjne	R3,#0,SpiSERR	;If R3 <> 0  or
		cjne	R1,#32,SpiS00	;If R1 < 32
SpiS00:	jc		SpiSERR			;  goto error routine
		mov		A,R1		; Load speed
...
SpiSERR:	mov	A,#7		;Send CR/LF
		lcall	30H
		mov	R3,#high spiemsg1	;Set string address.
		mov	R1,#low  spiemsg1
		ajmp	SPIERROR

;-------------------------------------------------
; SPIPUT   [byte] 
SPIPUT:		mov	A,#39H		; get argument "byte"
		lcall	30H			;
		mov	A,#1			;
		lcall	30H			; to R3:R1
		cjne	R3,#0,SpiPERR	;If R3 not zero
		mov	A,R1			; Load byte to send.

;	send data to SPI 
		mov	SPI0_DATA,A		; Send data
SpiP00: jnb	S0_FREE,SpiP00	; Wait for transmission complete
		mov	status,SPI0_DATA	; save Receive data to i-RAM

		ret					;Return to BASIC.

SpiPERR:	mov	A,#7		;Send CR/LF
		lcall	30H
		mov	R3,#high spiemsg2	;Set string address.
		mov	R1,#low  spiemsg2
		sjmp	SPIERROR

;-------------------------------------------------
; SPIGET   [variable]
SPIGET:	mov	SPI0_DATA,#00H	; Dummy transmission data
SpiG00: jnb	S0_FREE,SpiG00	; Wait for transmission complete
		mov	a,SPI0_DATA		; Receive data

		mov	R0,A			; set receive data to R0
		mov	R2,#0			; set 0 to R2
		mov	A,#9AH			; Put R2:R0 on argument stack
		lcall	30H			;

		mov	A,#43H			; Set next variable to value
		ljmp	30H			; on argument stack and return to BASIC.

;-------------------------------------------------
; SPIROT  ([byte]) [variable] 
SPIROT:	mov	A,#39H		; get argument "byte"
		lcall	30H			;
		mov	A,#1			;
		lcall	30H			; to R3:R1
		cjne	R3,#0,SpiROERR	;If R3 not zero
		mov	A,R1			; Load byte to send.

;	send & receive data to/from SPI
		mov	SPI0_DATA,A		; Send data
SpiRO0: jnb	S0_FREE,SpiRO0	; Wait for transmission complete
		mov	a,SPI0_DATA		; Receive data
...
SpiROERR:	mov	A,#7		;Send CR/LF
		lcall	30H
		mov	R3,#high spiemsg3	;Set string address.
		mov	R1,#low  spiemsg3
		sjmp	SPIERROR

;-------------------------------------------------
; SPICS [0/1]
SPICS:	mov	A,#39H			; get argument "0/1"
		lcall	30H			;
		mov	A,#1			;
		lcall	30H			; to R3:R1

		mov	A,R1			; Load "0/1"
		cjne	A,#1,SpiC00	; If R1 = 1 ?
;		orl		P1,#00010000B	; SCS High
		setb	SCS				; SCS High
		ret

SpiC00:	cjne	A,#0,SpiCERR	; If R1 = 0 ?
;		anl		P1,#11101111B	; SCS Low
		clr		SCS				; SCS Low
		ret

SpiCERR:	mov	A,#7		;Send CR/LF
		lcall	30H
		mov	R3,#high spiemsg4	;Set string address.
		mov	R1,#low  spiemsg4

;-------------------------------------------------
SPIERROR:	setb	34H		;Read string from code memory.
		mov	A,#6			;Send String to
		lcall	30H			;terminal.
		clr	A
		ljmp	30H			;Back to command mode.

spiemsg1:	db	'ERROR: BAD SPISTART ARGUMENT'
		db		22H			;End of text.

spiemsg2:	db	'ERROR: BAD SPIPUT ARGUMENT'
		db		22H			;End of text.

spiemsg3:	db	'ERROR: BAD SPIROT ARGUMENT'
		db		22H			;End of text.

spiemsg4:	db	'ERROR: BAD SPICS ARGUMENT'
		db		22H			;End of text.

; === SPI command processer end ==================


; === Software I2C command processer =============
; Re-write all I2C routine 2023.06.18
...
;----- Read acknowledge from slave --------------
		mov		status,#0	; clear error code
		setb	sda			; pull-up for read
		nop
		nop
		setb	scl
		acall	delay
		jnb		SDA,ackok	;Check for valid acknowledge.
		mov		status,#1	;Set error code.
...
;----- Get acknowledge and check 0/1 -------------
		mov		A,#39H		; get argument acknowledge
		lcall	30H			;
		mov		A,#1		;
		lcall	30H			; to R3:R1

		cjne	r3,#0,i2cerror	; If R3 not zero, then value bigger than 255.
		cjne	r1,#1,i2cg00	; If R1 = 1 ?
		sjmp	i2cg01
i2cg00:	cjne	r1,#0,i2cerror	; If R1 = 0 ?

;----- Receive byte from slave -------------------
i2cg01:	setb	sda			; pull-up for read
		mov		R6,#8		; Load bit counter
		clr		a			; clear acc
...
read:	acall	delay
		...
		mov		R0,A		;Load R0 with received byte.

;----- Send acknowledge to slave ----------------
		acall	delay
		mov		A,r1		; Load acknowledge
		rrc		A			;into C and
		mov		SDA,C		;send acknowledge bit.
		nop
		nop
		setb	scl	        ;Send acknowledge clock.
		acall	delay
		clr		scl
		nop
		nop
...
;----- delay about 2.5uS --------------------------
		org		$/2*2+1		; set odd address location

delay:	mov		r7,#18		; loop count at Fsys=48MHz
delay1:	djnz	r7,delay1	; ! place this instruction at an odd address
		ret
...
i2cerrmsg:
		db		'ERROR: BAD I2CPUT/I2CGET ARGUMENT'
		db		22H			;End of text.  	

; === Software I2C command processer end =========

;
; === ADC command processer ======================
;
; ADCSTART [channel] 

ADCSTART:
		mov	A,#39H				; get argument "channel"
		lcall	30H				;
		mov	A,#1				;
		lcall	30H				; to R3:R1
		cjne	R3,#0,AdcSERR	;If R3 <> 0
		cjne	R1,#8,AdcS00	;If R1 >= 8
AdcS00:	jnc		AdcSERR			;  goto error routine

;---- create analog port mask-bit ----------------
		mov	ar7,r1				; analog port-no
		mov	r6,#01h				; initial port mask-bit = 0b00000001

AdcS01:	mov	a,r7				; loop-no
		jz	AdcS02				; shift loop end ?

		mov	a,r6				; shift left mask-bit
		rl	a
		mov	r6,a				; R6 = new mask-bit

		dec	r7
		sjmp	AdcS01

;---- setup ADC function register ----------------
AdcS02:	mov	a,r6				; R6 = mask-bit
		cpl	a					; not mask-bit
		mov	P1_IE,a				; set P1.x(AINx) is analog port

		orl	ADC_SETUP,#04h		; ADC power on
		mov	ADC_CK_SE,#12		; set frequency division, 48MHz/12 = 4MHz
		mov	ADC_CTRL,#00h		; set manual sampling and manual channnel selection mode
		mov	ADC_CHANN,r6		; use analog port P1.x(AINx)
		mov	ADC_EX_SW,#04h		; set 11bit sampling

		acall	delay100		; wait 100uS

		mov	a,ADC_FIFO_L		; clear FIFO, dummy read
		mov	a,ADC_FIFO_H

		orl	ADC_STAT,#10h		; clear bADC_IF_ACT flag

		ret						; Return to BASIC.

;---- wait about 100uS ---------------------------
delay100:
		acall	delay25			; wait 25uS
		acall	delay25
		acall	delay25
		acall	delay25
		ret
...
delay25:	mov		r7,#180		; wait 25uS at Fsys=48MHz
delay25a:	djnz	r7,delay25a	; ! place this instruction at an odd address
		ret
...
ADCSTOP:
		anl	ADC_SETUP,#0fbh		; ADC power off
		mov	P1_IE,#0ffh			; set all P1 port is digital in/out (defalt)
...
;---- generate ADC sampling pulse ----------------
		orl	ADC_CTRL,#80h		; set 1 to bADC_SAMPLE, manually generate sampling pulse
		acall	delay			; wait 2.5uS
		acall	delay			; wait 2.5uS
		anl	ADC_CTRL,#7fh		; set 0 to bADC_SAMPLE, sampling start

;---- wait for conversion complete ---------------
AdcG00:	mov	a,ADC_STAT
		jnb	acc.4,AdcG00		; wait for bADC_IF_ACT flag = 1

		orl	ADC_STAT,#10h		; clear bADC_IF_ACT flag

		mov	r0,ADC_FIFO_L		; get ADC value low-byte
		mov	r2,ADC_FIFO_H		; get ADC value High-byte
...
AdcSERR:	mov	A,#7			;Send CR/LF
		lcall	30H
		mov	R3,#high adcerrmsg	;Set string address.
		mov	R1,#low adcerrmsg
...
adcerrmsg:	db	'ERROR: BAD ADCSTART ARGUMENT'
		db	22H					;End of text.

; === ADC command processer end ==================
;
; === Pulse generator command processer (for test) =====
;
; PULSE [loop-number] 

PULSE:
		mov	A,#39H				; get argument "loop-number"
		lcall	30H				;
		mov	A,#1				;
		lcall	30H				; to R3:R1
...
PUL000:	mov		a,r3			;If R3(high-byte) = 0 ?
		jz		PUL010			; yes, skip high-byte loop
		mov		ar7,r3			; high-byte loop(256 times loop) number

PUL002:	mov		a,#0			; 256 times loop
PUL003:	nop
		nop
		dec		a
		jnz		PUL003			; 256 times end ?
		djnz	r7,PUL002		; R3 times end ?

PUL010:	mov		a,r1			;If R1(low-byte) = 0 ?
		jz		PUL020			; yes, skip low-byte loop

PUL012:	nop						; R1 times loop
		nop
		dec		a
		jnz		PUL012			; R1 times end ?

;----- Port SCK(P1.7) toggle ---------------------
PUL020:	cpl		scl				; port SCL(P1.7) toggle
		jb		sda,PUL000		; loop continue ? (port SDA(P1.6) is on ? )

		ret						; Return to BASIC.

; === Pulse generator command processer end ======

; === SFR command processer ======================
